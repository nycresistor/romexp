...
impl Font {
    pub fn new() -> Font {
        ...
        let program =
            glutil::build_program(VS_SRC, FS_SRC).expect("Failed to create font shader program");
    ...
    pub fn width(&self, text: &str) -> i32 {
        self.size(text).0 as i32
    }
    pub fn height(&self, text: &str) -> i32 {
        self.size(text).1 as i32
    }

    pub fn draw(&self, window_size: (i32, i32), text_pos: (i32, i32), text: &str) {
        let charw = 1.0 / 128.0;
        // chars are 8x10
